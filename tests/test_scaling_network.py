...
def setup_network():
    ...
    neuron_numbers = NeuronNumbers(surface_area=net_params["surface_area"], **net_params["cytoarchitecture_params"])
    synapse_numbers = SynapseNumbers(NN=neuron_numbers, **net_params["predictive_connectomic_params"])

    # Get full-scale neuron and synapse numbers
    fullscale_NN_SN = {"NN": neuron_numbers.getNeuronNumbers(), "SN": synapse_numbers.getSynapseNumbers()}
...
def test_scaling_factors(setup_network, capsys):
    ...
    assert np.all(
        net_dict["neuron_numbers"] == expected_neurons
    ), "Neuron numbers do not match expected downscaled values."
    assert np.all(
        net_dict["synapses_internal"] == expected_synapses
    ), "Synapse numbers do not match expected downscaled values."
