...
class Simulation:
        ...
        with open(pop_file_name, "w+") as pop_file:
            for pop, nn in self.net_dict["neuron_numbers"].items():
                if nn > 0:
                    ...
                    # Assign DC input
                    nest.SetStatus(population, {"I_e": self.net_dict["dc_drive"].loc[pop]})

                    population.set(neuron_params_pop)
                    population.V_m = nest.random.normal(mean=self.sim_dict["V0_mean"], std=self.sim_dict["V0_sd"])
                    # Neuron parameters
                    for prm, dist_dict in nrn_prm_dist_pop.items():
                        if dist_dict["rel_sd"] > 0:
                            ...
                            if param_dist == "lognormal":
                                ...
                                assert mean_prm > 0
                                mu_param, sigma_param = mu_sigma_lognorm(mean=mean_prm, rel_sd=dist_dict["rel_sd"])
                                population.set(
                                    {prm: offset_prm + nest.random.lognormal(mean=mu_param, std=sigma_param)}
                                )
                            else:
                    ...
                    pop_file.write(
                        "{};{};{}\n".format(pop, population[0].get()["global_id"], population[-1].get()["global_id"])
                    )
        print("Memory on rank {} after creating populations: {:.2f}MB".format(nest.Rank(), self._getMemoryMB()))
    ...
    def create_devices(self):
        ...
        """
        if "spike_recorder" in self.sim_dict["rec_dev"]:
            recdict = {"record_to": "ascii", "label": os.path.join(self.spike_path, "spike_recorder")}
    ...
    def connect_neurons(self):  # noqa: C901
        ...
        for (area_i, layer_i, pop_i), target_pop in self.pops.items():
            for (area_j, layer_j, pop_j), source_pop in self.pops.items():
                synapse_nr = self.net_dict["synapses_internal"].loc[(area_i, layer_i, pop_i), (area_j, layer_j, pop_j)]
                ...
                if synapse_nr > 0.0:
                    if area_i == area_j:
                        ...
                        min_delay = max(self.sim_dict["delay_cc_min"], self.sim_dict["sim_resolution"])
                        mean_delay = self.net_dict["delay_cc"].loc[area_i, area_j]
                        std_delay = self.net_dict["delay_cc_sd"].loc[area_i, area_j]
                    ...
                    if delay_distr == "normal_clipped":
                        ...
                        mu_delay, sigma_delay = mu_sigma_lognorm(mean=mean_delay, rel_sd=std_delay / mean_delay)
                    else:
                        err_msg = f"Delay distribution {delay_distr}"
                        err_msg += " not implemented."
                        raise NotImplementedError(err_msg)
                    weight = self.net_dict["weights"].loc[(area_i, layer_i, pop_i), (area_j, layer_j, pop_j)]
                    w_sd = self.net_dict["weights_sd"].loc[(area_i, layer_i, pop_i), (area_j, layer_j, pop_j)]
                    if conn_rule == "fixed_total_number":
                        conn_dict_rec = {"rule": conn_rule, "N": synapse_nr}
                    elif conn_rule == "fixed_indegree":
                        neuron_nr = self.net_dict["neuron_numbers"].loc[(area_i, layer_i, pop_i)]
                        indegree = int(np.round(synapse_nr / neuron_nr))
                        conn_dict_rec = {"rule": conn_rule, "indegree": indegree}
                    else:
                    ...
                    if np.isclose(self.net_dict["p_transmit"], 1):
                        ...
                        syn_dict = {"synapse_model": "bernoulli_synapse", "p_transmit": self.net_dict["p_transmit"]}

                    if delay_distr == "normal_clipped":
                        syn_dict["delay"] = nest.math.max(nest.random.normal(mean=mu_delay, std=sigma_delay), min_delay)
                    elif delay_distr == "lognormal_clipped":
                        syn_dict["delay"] = nest.math.max(
                            nest.random.lognormal(mean=mu_delay, std=sigma_delay), min_delay
                        )

                    # Skipping connection with weight == 0.0
                    if weight == 0.0:
                        print(
                            "Found weight == 0.0 between {} and {}. Skipping connection.".format(
                                (area_i, layer_i, pop_i), (area_j, layer_j, pop_j)
                            )
                        )
                        continue
            ...
            print(
                ...
                "on rank {}. Memory: {:.2f} MB.".format(area_i, layer_i, pop_i, nest.Rank(), self._getMemoryMB())
            )
        print("Memory on rank {} after creating connections: {:.2f}MB".format(nest.Rank(), self._getMemoryMB()))
    ...
    def connect_poisson(self):
        ...
        for (area_i, layer_i, pop_i), target_pop in self.pops.items():
            ...
            weight = self.net_dict["weights_ext"].loc[(area_i, layer_i, pop_i)]
            w_sd = self.net_dict["weights_ext_sd"].loc[(area_i, layer_i, pop_i)]
            syn_dict_poisson = {
            ...
            nest.Connect(
                self.poisson[(area_i, layer_i, pop_i)],
                target_pop,
    ...
    def connect_single_spike(self):
        ...
        for pop, spike in self.single_spike.items():
            ...
            nest.Connect(spike, target_pop, syn_spec=syn_dict_single_spike)

    def connect_devices(self):
        """Connects the recording devices to the microcircuit."""
        if nest.Rank() == 0:
            if "spike_recorder" in self.sim_dict["rec_dev"] and "voltmeter" not in self.sim_dict["rec_dev"]:
                print("Spike detector connected")
            elif "spike_recorder" not in self.sim_dict["rec_dev"] and "voltmeter" in self.sim_dict["rec_dev"]:
                print("Voltmeter connected")
            elif "spike_recorder" in self.sim_dict["rec_dev"] and "voltmeter" in self.sim_dict["rec_dev"]:
    ...
    def _getMemoryMB(self):
        ...
        """
        try:
            currMem = nest.ll_api.sli_func("memory_thisjob") / 1024.0
        except AttributeError:
            currMem = nest.sli_func("memory_thisjob") / 1024.0
        return currMem
