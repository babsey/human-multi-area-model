...
class Analysis:
    ...
    def __init__(self, ana_params, net_dict, sim_dict, sim_folder, base_path):
    ...
    def fullAnalysis(self):
        ...
        print("{} Calculating correlation coefficients.".format(datetime.now().time()))
        ...
        print("{} Calculating synaptic input currents".format(datetime.now().time()))
        self.curr_in = self.synapticInputCurrent()

        print("{} Calculating rates of all neuron".format(datetime.now().time()))
        self.individual_rates = self.individualFiringRate()
        print("{} Calculating binned spike rates per neuron".format(datetime.now().time()))
        self.spikes_per_neuron_population_resolved, self.spikes_per_neuron_area_resolved = (
            self.binned_spikerates_per_neuron()
        )
        ...
        self.plotPopLV()
        print("{} Plotting {}".format(datetime.now().time(), "Correlation coefficients"))
        self.plotCorrCoff()
        print("{} Plotting binned spike rates per neuron".format(datetime.now().time()))
        self.plot_all_binned_spike_rates_area()

        print("{} Plotting functional connectivity based on synaptic input currents".format(datetime.now().time()))
        self.plot_functional_connectivity(save_fig=True)
        print("{} Plotting {}".format(datetime.now().time(), "area averaged spike rates"))
        self.plotAllFiringRatesSummary()
        print("{} Plotting {}".format(datetime.now().time(), "Synaptic currents"))
        self.plotAllSynapticCurrentsSummary()
        print("{} Plotting {}".format(datetime.now().time(), "BOLD signal"))
    ...
    def meanFiringRate(self):
        ...
        try:
            ...
            t_sim_sec = (t_stop - t_start) / 1000.0  # Convert ms to s
            ...
            # the simulation time.
            rate = (
                self.spikes.apply(lambda sts: sum([st[(st >= t_start) & (st < t_stop)].size for st in sts])).div(
                    self.popGids["pop_size"]
                )
                / t_sim_sec
            )
    ...
    def individualFiringRate(self):
        ...
        try:
            ...
            rate = (
                self.spikes.apply(lambda sts: np.array([st[(st >= t_start) & (st < t_stop)].size for st in sts]))
                / t_sim_sec
            )
    ...
    def binned_spikerates_per_neuron(self):
        ...
        """
        try:
            spikes_per_neuron_population_resolved = pd.read_pickle(
                os.path.join(self.sim_folder, "spikes_per_neuron_population_resolved.pkl")
            )
            spikes_per_neuron_area_resolved = pd.read_pickle(
                os.path.join(self.sim_folder, "spikes_per_neuron_area_resolved.pkl")
            )
        except FileNotFoundError:
            ...
            spikes_per_neuron_area_resolved_tmp = (
                self.spikes.apply(list)
                .groupby("area")
                .agg(sum)
                .apply(lambda tmp: np.histogram(np.array([len(i) for i in tmp]), bins=bins))
            )
            ...
            spikes_per_neuron_population_resolved.to_pickle(
                os.path.join(self.sim_folder, "spikes_per_neuron_population_resolved.pkl")
            )
            spikes_per_neuron_area_resolved.to_pickle(
                os.path.join(self.sim_folder, "spikes_per_neuron_area_resolved.pkl")
            )
    ...
    def firingRateHistogram(self):
        ...
        """
        try:
            rate_hist = pd.read_pickle(os.path.join(self.ana_folder, "rate_histogram.pkl"))
            rate_hist_areas = pd.read_pickle(os.path.join(self.ana_folder, "rate_histogram_areas.pkl"))
        except FileNotFoundError:
            ...
            rate_hist_areas = (
                ...
                .groupby("area")
                .agg(sum)
                .apply(calc_rates, args=(self.sim_dict, self.ana_dict))
                .div(self.popGids.groupby("area").agg(sum)["pop_size"])
            )

            rate_hist.to_pickle(os.path.join(self.ana_folder, "rate_histogram.pkl"))

            rate_hist_areas.to_pickle(os.path.join(self.ana_folder, "rate_histogram_areas.pkl"))
    ...
    def plot_instantaneous_firing_rate(self, save_fig=False):
        ...
        sns.heatmap(
            rate_hist_areas_df, cmap="YlOrBr", cbar_kws={"label": "Spikes/s"}, yticklabels=rate_hist_areas_df.index
        )
        plt.xlabel("Time (ms)")
        plt.ylabel("Area")
    ...
    def synapticInputCurrent(self):
        ...
        """
        try:
            curr = pd.read_pickle(os.path.join(self.ana_folder, "input_current.pkl"))
        except FileNotFoundError:
            # Get parameters
            ...
            for (area, layer, population), dat in inst_rates.items():
                ...
                tmp[(area, layer, population)] = convolve(tau_s * dat, kernel, mode="same")
            ...
            curr.to_pickle(os.path.join(self.ana_folder, "input_current.pkl"))
    ...
    def computeBOLD(self):
        ...
        try:
            BOLDSIGNAL = pd.read_pickle(os.path.join(self.ana_folder, "bold_signal.pkl"))
        except FileNotFoundError:
            if not hasattr(self, "rate_hist_areas"):
                _, self.rate_hist_areas = self.firingRateHistogram()
            rate_hist_areas = self.rate_hist_areas * 1000.0  # convert
            # spikes / [ms] to
            # spikes / [s]

            def calculate_BOLD(rate, area=None):
                # Assuming the rate is binned in [ms]
                t_vals = np.arange(0, len(rate)) / 1000.0
                ...
                z_val = interp1d(t_vals, rate, bounds_error=False, fill_value="extrapolate")
                ...
                def balloon_windkessel(t, w, z):
                    ...
                    * K.J. Friston, Katrin H. Preller, Chris Mathys, Hayriye
                    ...
                    kappa = 0.65  # time-constant for signal decay or
                    ...
                    gamma = 0.41  # time-constant for autoregulatory feedback
                    ...
                    # this value reversed, is this a bug on their
                    # or our side? cf tvb, wholebrain, deco
                    tau = 0.98  # Mean transit time [s], time to traverse the
                    # venous compartment
                    alpha = 0.32  # Grubb's exponent, Grubb et al 1974
                    rho = 0.34  # oxygen extraction fraction, sometimes called
                    # E0
                ...
                balloon_windkessel_solution = solve_ivp(
                    lambda t, w: balloon_windkessel(t, w, z_val), (t_vals[0], t_vals[-1]), w0
                )

                _, _, v, q = balloon_windkessel_solution.y
                time = balloon_windkessel_solution.t

                rho = 0.34  # oxygen extraction fraction, sometimes called E0
            ...
            bold = Parallel(n_jobs=-1)(delayed(calculate_BOLD)(rate, area) for area, rate in rate_hist_areas.items())
            ...
            BOLDSIGNAL.to_pickle(os.path.join(self.ana_folder, "bold_signal.pkl"))
        return BOLDSIGNAL
    ...
    def plotBoxPlot(self):
        ...
        plt.ylabel("Population")
        fig.savefig(os.path.join(self.plot_folder, "boxplot.{0}".format(extension)))
        plt.clf()
        plt.close(fig)
    ...
    def popCorrCoeff(self):
        ...
        """
        try:
            cc = pd.read_pickle(os.path.join(self.sim_folder, "cc.pkl"))
        except FileNotFoundError:
    ...
    def plotCorrCoff(self):
        ...
        plt.ylabel("Population")
        fig.savefig(os.path.join(self.plot_folder, "cc.{0}".format(extension)))
        plt.clf()
        plt.close(fig)
    ...
    def popLv(self):
        ...
        """
        try:
            lv = pd.read_pickle(os.path.join(self.sim_folder, "lv.pkl"))
        except FileNotFoundError:
            ...
            t_ref = t_ref_E
            lv = self.spikes.apply(LV, args=(t_ref, t_start, t_stop, min_spikes))
    ...
    def plotPopLV(self):
        ...
        plt.ylabel("Population")
        fig.savefig(os.path.join(self.plot_folder, "lv_isi.{0}".format(extension)))
        plt.clf()
        plt.close(fig)
    ...
    def popCvIsi(self):
        ...
        """
        try:
            cv_isi = pd.read_pickle(os.path.join(self.sim_folder, "cv_isi.pkl"))
        except FileNotFoundError:
            ...
            cv_isi = self.spikes.apply(cvIsi, args=(t_start, t_stop, min_spikes))
    ...
    def plotPopCVIsi(self):
        ...
        plt.ylabel("Population")
        fig.savefig(os.path.join(self.plot_folder, "cv_isi.{0}".format(extension)))
        plt.clf()
        plt.close(fig)
    ...
    def plot_functional_connectivity(self, save_fig=False):
        ...
        exp_data_exists = os.path.exists(os.path.join(data_dir, "ROIs.txt")) and os.path.exists(
            os.path.join(data_dir, "rsDATA_7T_DKparcel.npy")
        )

        if exp_data_exists:
            # Read in regions of interest.
            roi = pd.read_csv(os.path.join(data_dir, "ROIs.txt"), header=None, names=["roi"], dtype=str).squeeze()
        ...
        if save_fig:
            extension = self.ana_dict["extension"]
            fig.savefig(os.path.join(self.plot_folder, "simulated_synaptic_currents_correlation.{0}".format(extension)))
            plt.close(fig)
    ...
    def calculateBOLDConnectivity(self):
        ...
        """
        try:
            BOLD_correlation = pd.read_pickle(os.path.join(self.sim_folder, "bold_correlation.pkl"))
        except FileNotFoundError:
            if not hasattr(self, "self.BOLD"):
                self.BOLD = self.computeBOLD()
            # Start and end time, cast to [s]
            t_min = self.ana_dict["plotBOLD"]["tmin"] / 1000.0
            t_max = self.sim_dict["t_sim"] / 1000.0
            ...
            # to the same grid.
            bold_timesteps = self.ana_dict["plotBOLD"]["stepSize"] / 1000.0
            t_new = np.arange(t_min, t_max, bold_timesteps)
            ...
            for area, data in self.BOLD.groupby(level=0):
                ...
                bold_fun = interp1d(t, b, bounds_error=False, fill_value="extrapolate")
            ...
            BOLD_correlation = pd.DataFrame(index=index, columns=index, dtype=np.float64)
            for source in index:
                for target in index:
                    BOLD_correlation.loc[source, target] = np.corrcoef(
                        [BOLD_RESAMPLED.loc[source, "bold"], BOLD_RESAMPLED.loc[target, "bold"]]
                    )[0, 1]
            BOLD_correlation.to_pickle(os.path.join(self.sim_folder, "bold_correlation.pkl"))
    ...
    def calculateFuncionalConnectivityCorrelations(self):
        ...
        df_sim_fc_bold = df_sim_fc_bold.sort_index(axis=0).sort_index(axis=1)

        # Derive numpy array containing the correlation values. Handier than
        # the underlying pandas dataframe
        sim_fc_bold = df_sim_fc_bold.values[np.tril_indices(df_sim_fc_bold.shape[0])]
        sim_fc_bold = sim_fc_bold[~np.isnan(sim_fc_bold)]
        sim_fc_syn = df_sim_fc_syn.values[np.tril_indices(df_sim_fc_syn.shape[0])]
        ...
        sim_fc_bold__sim_fc_syn__corr = np.corrcoef([sim_fc_bold, sim_fc_syn])[0, 1]

        # This is PEP 8 conform, but is it really nice?
        outfile.write(
            ("correlation_simulated_bold_simulated_functional_connectivity" f": {sim_fc_bold__sim_fc_syn__corr}\n")
        )
        ...
        try:
            ...
            for i in range(no_of_persons):
                tmp_lh = exp_fc[i]["lh"].values[np.tril_indices(exp_fc[i]["lh"].shape[0])]
                tmp_rh = exp_fc[i]["rh"].values[np.tril_indices(exp_fc[i]["rh"].shape[0])]
            ...
            outfile.write(("exp__exp__corr_lh:\n" f"{exp__exp__corr_lh}\n"))
            outfile.write(("exp__exp__corr_rh:\n" f"{exp__exp__corr_rh}\n"))
            outfile.write(("exp_fc__sim_fc_syn__array_lh:\n" f"{exp_fc__sim_fc_syn__array_lh}\n"))
            outfile.write(("exp_fc__sim_fc_syn__array_rh:\n" f"{exp_fc__sim_fc_syn__array_rh}\n"))
            outfile.write(("exp_fc__sim_fc_bold__array_lh:\n" f"{exp_fc__sim_fc_bold__array_lh}\n"))
            outfile.write(("exp_fc__sim_fc_bold__array_rh:\n" f"{exp_fc__sim_fc_bold__array_rh}\n"))
            outfile.write(("exp__exp_mean__corr_lh: " f"{exp__exp_mean__corr_lh}\n"))
            outfile.write(("exp__exp_mean__corr_rh: " f"{exp__exp_mean__corr_rh}\n"))

            # Calculate experimental mean functional connectivity
            exp_fc_mean_lh = (
                np.sum(
                    [exp_fc[i]["lh"].values[np.tril_indices(exp_fc[i]["lh"].shape[0])] for i in range(no_of_persons)],
                    axis=0,
                )
                / no_of_persons
            )
            exp_fc_mean_rh = (
                np.sum(
                    [exp_fc[i]["rh"].values[np.tril_indices(exp_fc[i]["rh"].shape[0])] for i in range(no_of_persons)],
                    axis=0,
                )
                / no_of_persons
            )
            ...
            exp__sim_bold__corr_lh = np.corrcoef([exp_fc_mean_lh, sim_fc_bold])[0, 1]
            exp__sim_bold__corr_rh = np.corrcoef([exp_fc_mean_rh, sim_fc_bold])[0, 1]

            outfile.write(("exp__sim_syn__corr_lh: " f"{exp__sim_syn__corr_lh}\n"))
            outfile.write(("exp__sim_syn__corr_rh: " f"{exp__sim_syn__corr_rh}\n"))
            outfile.write(("exp__sim_bold__corr_lh: " f"{exp__sim_bold__corr_lh}\n"))
            outfile.write(("exp__sim_bold__corr_rh: " f"{exp__sim_bold__corr_rh}"))
        except FileNotFoundError:
            print(
                (
                    "Cannot load experimantal FMRI data. "
    ...
    def calculateRateDistributionSimilarity(self):
        ...
        try:
            ...
            # =================================================================
            # Set Path to experimental data
            data_dir = os.path.join(self.base_path, "experimental_data", "rutishauser", "spikes")
            ...
            if self.ana_dict["rate_distribution_similarity"]["remove_zeros"]:
                rates_data = [rate for rate in rates_data if not np.isclose(rate, 0)]
                rates_sim_area = rates_sim_area.apply(lambda rates: [rate for rate in rates if not np.isclose(rate, 0)])
            # Compute KS distance
            KS_stats = rates_sim_area.apply(ks_2samp, args=(rates_data,))
            # Split tuples and write to file
            KS_stats = pd.DataFrame(data=KS_stats.values.tolist(), index=KS_stats.index, columns=["KS_dist", "p_value"])
            KS_stats.to_csv(outfile, sep="\t")
        except FileNotFoundError:
            print(
                (
                    "Cannot load experimantal spiking data. "
                    "Either the the datapath is wrong or the data is not available."
                )
            )
    ...
    def plotBOLDConnectivity(self):
        ...
        fig = plt.figure(1)
        sns.heatmap(
            self.BOLD_correlation, square=True, cmap="YlGnBu", cbar=True, cbar_kws={"shrink": 0.66}, vmin=-1.0, vmax=1.0
        )
        plt.tight_layout()
        fig.savefig(os.path.join(self.plot_folder, "bold_correlation.{0}".format(extension)))
        plt.clf()
        plt.close(fig)
    ...
    def plotRasterArea(self, area):
        ...
        for (layer, pop), sts in self.spikes.loc[area].items():
            ...
            pop_size = self.popGids.loc[area, layer, pop].maxGID - self.popGids.loc[area, layer, pop].minGID + 1

            # Fraction of total number of neurons
            no_sts = int(fraction * pop_size)
            # Fraction of neurons that actually spiked
            frac_spiking = len(sts) * 1.0 / pop_size
        ...
        ax1.set_title(area)
        fig.savefig(os.path.join(self.plot_folder, "raster_{0}.{1}".format(area, extension)))
        plt.clf()
        plt.close(fig)
    ...
    def plot_raster_statistics(self, save_fig=False, raster_areas=None):  # noqa: C901
        ...
        for ax, data, label in zip(axs_boxplots, data_boxplots, boxplots_labels):
            ...
            boxplot = sns.boxplot(data=data_lp, orient="h", ax=ax, saturation=1, width=0.5, fliersize=2.5, color="k")
    ...
    def plot_all_binned_spike_rates_area(self):
        ...
        fig, axes = plt.subplots(num_row, num_col, figsize=(22, 22))
        ...
        for num, (area, (tmp_count, tmp_rates)) in enumerate(signal.groupby(level=0)):
            ...
            # Plot the lineplot
            ax.bar(rates, count, width=rates[1] - rates[0], edgecolor="black", align="edge")
            ...
            # Add title
            ax.set_title(area, loc="left", fontsize=20, fontweight=0, color="black")

        fig.tight_layout()
        plt.savefig(os.path.join(self.plot_folder, "binned_spikerates_per_neuron_area_resolved.png"))
        plt.clf()
        plt.close(fig)
        plt.style.use("default")
    ...
    def plotAllSynapticCurrentsSummary(self):
        ...
        for same_axis in [True, False]:
            ...
            fig.tight_layout()
            plt.savefig(os.path.join(self.plot_folder, f"synaptic_currents_overview_same_axis_{same_axis}.png"))
            plt.clf()
            plt.close(fig)
            plt.style.use("default")
    ...
    def plotAllBOLDSignalSummary(self):
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(self.plot_folder, "bold_overview.png"))
        plt.clf()
        plt.close(fig)
        plt.style.use("default")
    ...
    def plotAllFiringRatesSummary(self):
        ...
        if not hasattr(self, "rate_hist_areas"):
            self.rate_hist, self.rate_hist_areas = self.firingRateHistogram()
        rate_hist_areas = self.rate_hist_areas * 1000.0  # ms to s
        ...
        times = np.arange(t_start, t_stop, resolution) / 1000.0  # To seconds
        ...
        for same_y_axis in [False, True]:
            ...
            fig.tight_layout()
            plt.savefig(os.path.join(self.plot_folder, "spiking_overview_same_y_axis_{}.png".format(same_y_axis)))
            plt.clf()
            plt.close(fig)
        plt.style.use("default")
    ...
    def _readSpikes(self):
        ...
        """
        try:
            spikes = pd.read_pickle(os.path.join(self.sim_folder, "spikes.pkl"))
        except FileNotFoundError:
    ...
    def _readPopGids(self):
        ...
        """
        try:
            popGids = pd.read_pickle(os.path.join(self.sim_folder, "population_GIDs.pkl"))
        except FileNotFoundError:
            popGids = pd.read_csv(
                os.path.join(self.sim_folder, "population_GIDs.dat"), sep=";", header=None, names=["minGID", "maxGID"]
            )
            popGids.index = pd.MultiIndex.from_tuples([literal_eval(s) for s in popGids.index])
    ...
    def _readSpikesFromDAT(self):  # noqa: C901
        ...
        if python_sort:
            ...
            for row in popGids.itertuples():
                ...
                last_gid = row.maxGID
                try:
                    first = np.where((spikes.gid <= last_gid) & (spikes.gid >= first_gid))[-1][0]
                    last = np.where((spikes.gid <= last_gid) & (spikes.gid >= first_gid))[-1][-1]
                    ...
                    pass
            spikes = spikes.groupby(["area", "layer", "pop"]).apply(lambda group: group.t.values)
            ...
            d_population_backward = {"23": "II/III", "4": "IV", "5": "V", "6": "VI"}
            ...
            for fn in dem_files:
                with open(fn) as f:
                    ...
                    exact_population = (
                        exact_population_tmp[0],
                        d_population_forward[exact_population_tmp[1]],
                    ...
                    new_name = os.path.join(self.rec_folder, "_".join(exact_population) + ".spiketrains")
...
def strip_binned_spiketrains(sp):
    ...
    """
    sp_stripped = np.array([x for x in sp if abs(np.max(x) - np.min(x)) > 1e-16])
    return sp_stripped
...
def shell_spiketrainify(fn):
    ...
    subprocess.check_output(lol, shell=True)
