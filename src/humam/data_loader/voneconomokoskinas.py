...
class VonEconomoKoskinas:
    ...
    def __init__(self, file_path):
        ...
        self.data = pd.read_excel(self.file_path, sheet_name="Sheet1", index_col=[0, 2])

        # Keep only cell density and layer thickness & rename
        self.data = self.data[["mean cell content per layer [cells/mm³]", "Layer thickness overall [mm]"]]
        self.data = self.data.rename(
            columns={
                "mean cell content per layer [cells/mm³]": "neuron_density",
                "Layer thickness overall [mm]": "layer_thickness",
            }
        )
        ...
        self.data["layer_thickness"] = self.data["layer_thickness"].astype(np.float64)
        self.data["neuron_density"] = self.data["neuron_density"].astype(np.float64)

        # Fill nas due to cell merge in layer thickness by 0
        self.data["layer_thickness"] = self.data["layer_thickness"].fillna(value=0)
        # Fill nas due to cell merge in neuron density by previous value
        self.data["neuron_density"] = self.data["neuron_density"].fillna(value=0)

    def __repr__(self):
        return (
            "Von Economo & Koskinas structural data\n"
            "File: {0}\nAreas (index lvl 0):\n{1}\n"
            "Layers (index lvl 1):\n{2}\nColumns:\n{3}\n".format(
                self.file_path,
