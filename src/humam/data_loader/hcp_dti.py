...
class HcpDesikanKilliany:
    ...
    def __init__(self, file_path):
        ...
        self.dist = pd.DataFrame(self.data["Dist"], columns=self.names, index=self.names, dtype=np.float64)

        self.RightHem = pd.DataFrame(
            self.data["LowResDK_EstimatedDist_RHem"], columns=self.names, index=self.names, dtype=np.float64
        )

        self.LeftHem = pd.DataFrame(
            self.data["LowResDK_EstimatedDist_LHem"], columns=self.names, index=self.names, dtype=np.float64
        )

        self.ConnectivityRight = pd.DataFrame(self.data["C_R"], columns=self.names, index=self.names, dtype=np.float64)

        self.ConnectivityLeft = pd.DataFrame(self.data["C_L"], columns=self.names, index=self.names, dtype=np.float64)

    def __repr__(self):
        return (
            "HCP connectivity data\n"
            "File: {0}\nRegions (major & minor axis):\n{1}\n"
            "Items:\n{2}\n".format(self.file_path, np.array(self.names), self.data.items.values)
        )
...
class VolumesDK:
    ...
    def __init__(self, file_path):
        ...
        self.names = np.concatenate(self.names.flatten()).tolist()
        self.data = pd.Series(data=self.data["Volume"].flatten(), index=self.names, dtype=np.float64)
