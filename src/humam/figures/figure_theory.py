...
def solve_theory(net_dict, t_arr, min_rate_0, max_rate_0):
    initial_rates = initial_rates_uniform(net_dict=net_dict, trials=1, seed=123, min=min_rate_0, max=max_rate_0)
...
def plot_results(rates, t_arr, rates_sim=None, rates_theo=None):
    ...
    ode_cbar = fig.colorbar(ode_mesh, ax=ax_ode, ticks=[-2, -1, 0, 1], pad=0.0, aspect=50, fraction=0.1)
    ...
    ax_ode.text(s="A", transform=ax_ode.transAxes, x=-0.0, y=1.2, **label_prms)
    ...
    ax_comp.text(s=r"$\rho$" + f" = {rate_corr:.3f}", x=0.2, y=0.8, transform=ax_comp.transAxes)
...
def meanfield_rate(outpath, net_hash, sim_hash=None):
    ...
    min_rate_0, max_rate_0 = 0.0, 10.0
