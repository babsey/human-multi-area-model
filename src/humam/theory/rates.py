...
def solve(net_dict, t, initial_rates):
    ...
    tau_syn = pd.DataFrame(data=0.0, index=N.index, columns=N.index)
    ...
    def aux_ode(_, nu_in):
        ...
        nu_out = _firing_rate_shift(
            V_0_rel=V_r, V_th_rel=V_th, mu=mu, sigma=sig, tau_m=tau_m, tau_r=t_ref, tau_s=tau_eff
        )
        return -nu_in + nu_out

    # solve auxilliary ODE with random initial conditions
    input_prms = dict(tau_m=tau_m, J=J, K=K, J_ext=np.diag(J_ext), K_ext=np.diag(K_ext), nu_ext=nu_ext)
    input_prms_scld = dict(
        tau_m=tau_m, J=J, K=K / tau_syn, J_ext=np.diag(J_ext), K_ext=np.diag(K_ext / tau_syn_ext), nu_ext=nu_ext
    )
    rates = {}
    for trial in initial_rates:
        sol_ivp = solve_ivp(fun=aux_ode, t_span=(t[0], t[-1]), y0=initial_rates[trial], method="LSODA", t_eval=t)
