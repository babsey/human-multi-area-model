...
from humam.data_loader.mapping_DK_vEK import dk_to_vEK as map_dk_to_vek
from humam.data_loader.ratio_exc_to_inh import ratio_exc_to_inh
from humam.data_loader.voneconomokoskinas import VonEconomoKoskinas
...
class NeuronNumbers:
    ...
    def __init__(  # noqa: C901
        ...
        if source == "VonEconomoKoskinas":
            ...
            layer_dict = {
                "I": ["I", "Ia", "Ib"],
                "II/III": ["II", "II+III", "III", "III(II)", "III(IV)", "IIIa", "IIIa/b", "IIIb", "IIIb/c", "IIIc"],
                "IV": ["IV", "IVa", "IVb", "IVc"],
            ...
            self.thickness = pd.Series(
                data=0,
                index=pd.MultiIndex.from_product([self.area_list, self.layer_list_plus1], names=["area", "layer"]),
                dtype=np.float64,
            )
            self.layerI_neurons = pd.Series(data=0, index=self.area_list, dtype=np.float64)

            # Map vEK layers to layer_list using layer_dict
            for area in self.area_list:
                for layer in self.layer_list:
                    layer_vEK = NeuronDensities[area].index.intersection(layer_dict[layer]).drop_duplicates()
                    if len(layer_vEK) > 0:
                        ...
                        self.density.loc[area, layer] = (
                            self.frac_pop[layer] * np.sum(density_vek * thickness_rel)
                        ).values
                    # Not in vEK data -> keep default value 0
                    else:
                        self.density.loc[area, layer] = 0
                for layer in self.layer_list_plus1:
                    layer_vEK = Layerthickness[area].index.intersection(layer_dict[layer]).drop_duplicates()
                    if len(layer_vEK) > 0:
                        self.thickness.loc[area, layer] = np.sum(Layerthickness[area].loc[layer_vEK])
                    # Not in vEK data -> keep default value 0
                    else:
                        self.thickness.loc[area, layer] = 0
                layer_vEK = NeuronDensities[area].index.intersection(layer_dict["I"]).drop_duplicates()
                if len(layer_vEK) > 0:
                    self.layerI_neurons.loc[area] = np.sum(
                        Layerthickness[area].loc[layer_vEK] * NeuronDensities[area].loc[layer_vEK] * self.surface_area
                    )
                # Not in vEK data -> keep default value 0
                else:
        ...
        for (area, layer), neuronnumber in neuron_sum.items():
            ...
            if remove_smaller_layerI and 0 < neuronnumber < nn_layerI:
                print(f"Dropping layer {layer} in {area} because it " "contains less neurons than layer I.")
                self.thickness.loc[area, layer] = 0
                self.density.loc[area, layer, :] = 0
            elif 0 < neuronnumber < min_neurons_per_layer:
                print(
                    f"Dropping layer {layer} in {area} because it "
    ...
    def map_atlas(self, source, target):
        ...
        if source == "VonEconomoKoskinas" and target == "DesikanKilliany":
            ...
            layerI_neurons_new = pd.Series(data=0, index=self.area_list, dtype=np.float64)
            ...
            for dk_area in self.area_list:
                ...
                thickness_new.loc[dk_area] = (
                    self.thickness.loc[map_dk_to_vek[dk_area]].groupby(level="layer").mean().values
                )
                ...
                density_weight = pd.Series(
                    data=np.repeat(
                        self.thickness.loc[map_dk_to_vek[dk_area], self.layer_list].values, len(self.population_list)
                    ),
                    index=density_dk_area.index,
                    dtype=np.float64,
                )
                density_weight /= np.tile(
                    density_weight.groupby(level=["layer", "population"]).sum().values,
                    len(density_weight.index.unique(level="area")),
                )
                density_weight = density_weight.fillna(0)  # absent layers
                density_new.loc[dk_area] = (
                    (density_dk_area * density_weight).groupby(level=["layer", "population"]).sum().values
                )
                # average layer 1 neuron number over all aggregated ares
                layerI_neurons_new.loc[dk_area] = self.layerI_neurons.loc[map_dk_to_vek[dk_area]].mean()
            self.density = density_new
            self.thickness = thickness_new
            self.layerI_neurons = layerI_neurons_new
        else:
            raise NotImplementedError("No mapping from {0} to {1} available.".format(source, target))
    ...
    def setPopulationDefault(self, fraction_E_neurons):
        ...
        self.frac_pop = pd.Series(
            data=0,
            index=pd.MultiIndex.from_product([self.layer_list, self.population_list], names=["layer", "population"]),
            dtype=np.float64,
        )
        for layer in self.layer_list:
            self.frac_pop.loc[layer, "E"] = fraction_E_neurons.loc[layer]
            self.frac_pop.loc[layer, "I"] = 1.0 - fraction_E_neurons.loc[layer]
    ...
    def getNeuronNumbers(self):
        ...
        thickness_repeat = pd.Series(
            data=np.repeat(self.thickness.loc[self.area_list, self.layer_list].values, len(self.population_list)),
            index=pd.MultiIndex.from_product(
                [self.area_list, self.layer_list, self.population_list], names=["area", "layer", "population"]
            ),
            dtype=np.float64,
        )
        return np.round(self.density * thickness_repeat * self.surface_area).astype(np.int64)
