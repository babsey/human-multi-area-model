...
# Experiment
exp_rates = np.load(os.path.join(save_data_dir, "exp_rates.npy"), allow_pickle=True)
...
for keys in ["groundstate", "bestfit"]:
    sim_rates[keys] = np.load(os.path.join(save_data_dir, f"sim_rates_{keys}.npy"), allow_pickle=True)
    sim_cv[keys] = np.load(os.path.join(save_data_dir, f"sim_cv_{keys}.npy"), allow_pickle=True)
    sim_lvr[keys] = np.load(os.path.join(save_data_dir, f"sim_lvr_{keys}.npy"), allow_pickle=True)
...
for key in ["groundstate", "bestfit"]:
    simulated_fc[key] = pd.read_csv(os.path.join(save_data_dir, f"simulated_fc_{key}.csv"), index_col=0, header=0)
...
gs = gridspec.GridSpec(3, 1, height_ratios=[1, 1, 2], bottom=0.1, top=0.95, left=0.1, right=0.95, hspace=0.5, wspace=1)
gs0 = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs[0], wspace=0.4)
gs1 = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs[1], hspace=1.0, wspace=0.4)
gs2 = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs[2], hspace=1.0, wspace=0.4)
...
df_ks_cv = pd.DataFrame({"chi": list(ks_cv_arr.keys()), "val": list(ks_cv_arr.values())}, dtype=float).sort_values(
    by="chi"
)

axes["ks"].plot(df_ks_cv["chi"], df_ks_cv["val"], label="CV ISI", color="#66CCEE", marker="x", linewidth=linewidth)

df_ks_lvr = pd.DataFrame({"chi": list(ks_lvr_arr.keys()), "val": list(ks_lvr_arr.values())}, dtype=float).sort_values(
    by="chi"
)

axes["ks"].plot(df_ks_lvr["chi"], df_ks_lvr["val"], label="LvR", color="#228833", marker="x", linewidth=linewidth)

df_ks_rates = pd.DataFrame(
    {"chi": list(ks_rates_arr.keys()), "val": list(ks_rates_arr.values())}, dtype=float
).sort_values(by="chi")

axes["ks"].plot(df_ks_rates["chi"], df_ks_rates["val"], color="#CCBB44", label="Rates", linewidth=linewidth)

axes["ks"].plot(
    df_corr_rh[True]["chi"], df_corr_rh[True]["val"], color="#EE6677", label="fMRI\n(Pearson)", linewidth=linewidth
)

axes["ks"].plot(
    df_corr_rh[False]["chi"], df_corr_rh[False]["val"], color="#4477AA", label="fMRI\n(RMSE)", linewidth=linewidth
)
...
axes["ks"].legend(loc="center left", bbox_to_anchor=(1.0, 0.5), frameon=False)

axes["ks"].text(s="A", transform=axes["ks"].transAxes, x=-0.085, y=1.15, **label_prms)
...
number_of_neurons_rates = min(len(exp_rates), len(sim_rates["bestfit"]), len(sim_rates["groundstate"]))
number_of_neurons_cv = min(len(exp_cv), len(sim_cv["bestfit"]), len(sim_cv["groundstate"]))
number_of_neurons_lvr = min(len(exp_lvr), len(sim_lvr["bestfit"]), len(sim_lvr["groundstate"]))
...
for type_ in ["groundstate", "bestfit"]:
    ...
    # Rates
    vals, bins = np.histogram(sim_rates[type_], bins=np.arange(0.5, 20.5, 0.5))
    vals = vals / np.sum(vals)
    axes["rates"].plot(bins[:-1], vals, color=colors_state[type_], linewidth=linewidth)
    ...
    axes["cv"].plot(x, cv_mean, color=colors_state[type_], linewidth=linewidth)

    axes["cv"].hist(sim_cv[type_], bins=np.linspace(0, 4, 100), color=colors_state[type_], alpha=0.5, density=True)
    ...
    axes["lvr"].plot(x, lvr_mean, color=colors_state[type_], label=label, linewidth=linewidth)

    axes["lvr"].hist(sim_lvr[type_], bins=np.linspace(0, 4, 100), color=colors_state[type_], alpha=0.5, density=True)
...
# Rates
vals, bins = np.histogram(exp_rates, bins=np.arange(0.5, 20, 0.5))
vals = vals / np.sum(vals)
axes["rates"].plot(bins[:-1], vals, color=colors_state["exp"], linewidth=linewidth)
...
axes["cv"].plot(x, cv_mean, color=colors_state["exp"], linewidth=linewidth)
...
axes["lvr"].plot(x, lvr_mean, color=colors_state["exp"], label="Experiment", linewidth=linewidth)
...
axes["rates"].text(s="B", transform=axes["rates"].transAxes, x=-0.3, y=1.2, **label_prms)

axes["cv"].set_xlim(0, 3)
axes["cv"].set_xlabel("CV ISI")
axes["cv"].text(s="C", transform=axes["cv"].transAxes, x=-0.15, y=1.2, **label_prms)
...
for person in range(no_of_persons):
    ...
    if person == 0:
        exp_fc = {"lh": lh_fc, "rh": rh_fc}
    else:
        exp_fc = {"lh": exp_fc["lh"] + lh_fc, "rh": exp_fc["rh"] + rh_fc}

exp_fc = {"lh": exp_fc["lh"] / no_of_persons, "rh": exp_fc["rh"] / no_of_persons}

im = axes["exp_bold"].pcolormesh(exp_fc["rh"].loc[ordering][ordering], vmin=-1, vmax=1, cmap="RdYlBu_r")

cbar_ticks = [-1, 0, 1]
cbar = plt.colorbar(im, ax=axes["exp_bold"], ticks=cbar_ticks)
...
axes["exp_bold"].text(s="E", transform=axes["exp_bold"].transAxes, x=-0.4, y=1.2, **label_prms)
...
for fmri_state in ["groundstate", "bestfit"]:
    ...
    im = axes[fmri_state].pcolormesh(df_sim_fc_syn.loc[ordering][ordering], vmin=-1, vmax=1, cmap="RdYlBu_r")

    cbar_ticks = [-1, 0, 1]
    cbar = plt.colorbar(im, ax=axes[fmri_state], ticks=cbar_ticks, shrink=0.7)
...
axes["groundstate"].text(s="F", transform=axes["groundstate"].transAxes, x=-0.17, y=1.2, **label_prms)

axes["bestfit"].text(s="G", transform=axes["bestfit"].transAxes, x=-0.17, y=1.2, **label_prms)
