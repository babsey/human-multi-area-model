...
def load_diffs(net_hash_orig, net_hash_pert):
    ...
    # load data
    rates_orig = pd.read_pickle(path_join(folder_orig, ana_hash, "rate_histogram.pkl"))
    rates_orig = pd.DataFrame(rates_orig.sort_index().to_dict())
    rates_pert = pd.read_pickle(path_join(folder_pert, ana_hash, "rate_histogram.pkl"))
...
def sload(net_hashes):
    ...
    for net_hash_A, net_hash_B in combinations(net_hashes, r=2):
        ...
        assert np.allclose(net_dict_A["neuron_numbers"], net_dict_B["neuron_numbers"])
        assert np.allclose(net_dict_A["synapses_internal"], net_dict_B["synapses_internal"])
...
for source, target in product(area_list, area_list):
    if source == target:
        delay_matrix.loc[(target, slice(None), slice(None)), (source, slice(None), "E")] *= delay_e
        delay_matrix.loc[(target, slice(None), slice(None)), (source, slice(None), "I")] *= delay_i
    else:
        delay_matrix.loc[(target, slice(None), slice(None)), (source, slice(None), slice(None))] *= delays_cc.loc[
            target, source
        ]

# compute shortest path based on mean delays
delay_graph = nx.from_numpy_array(delay_matrix.values.T, create_using=nx.DiGraph)  # A_ij from i to j
...
dist_pert = dist_matrix.loc[(slice(None), slice(None), slice(None)), pert_pop]
steps_pert = steps_matrix.loc[(slice(None), slice(None), slice(None)), pert_pop]
...
ax_delay.text(s="E", transform=ax_delay.transAxes, x=-0.05, y=1.4, **label_prms)

bins = np.arange(8) - 0.5
ax_steps.hist(steps_matrix.values.ravel(), bins=bins, color="black", log=True, rwidth=0.8)
ax_steps.set_xlabel("shortest path length")
ax_steps.set_title("Distance (steps)")
ax_steps.text(s="D", transform=ax_steps.transAxes, x=-0.05, y=1.4, **label_prms)
...
for ax_pert, rates_diff, label, title in zip(axs, diffs, labels, titles):
    ...
    ax_pert.text(s=label, transform=ax_pert.transAxes, x=-0.05, y=1.055, **label_prms)
    ...
    ax_pert.set_xticklabels((0, dt * (step_max - step_min) / 2, dt * (step_max - step_min)))
...
ax_time.plot(
    first_diff_min_gs.reindex(order, level=0),
    np.arange(len(pops)),
    "s",
    label="base version",
    color=colors_state["groundstate"],
)
ax_time.plot(
    first_diff_min_bf.reindex(order, level=0),
    np.arange(len(pops)),
    "s",
    label="best-fit version",
    color=colors_state["bestfit"],
)
...
ax_time.legend(loc="upper right", frameon=False, handletextpad=0.0)
ax_time.text(s="C", transform=ax_time.transAxes, x=-0.05, y=1.055, **label_prms)
