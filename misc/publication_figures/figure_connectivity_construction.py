...
from humam.data_loader.synapse_cellbody_probability import mohan
...
mohan_extended = pd.DataFrame(data=np.nan, index=[synapse_layer[0]], columns=synapse_layer).append(mohan)
...
# SLN data
syn_vek = net_dict["synapses_internal"].groupby(level=["area"], axis=0).sum().groupby(level=["area"], axis=1).sum()
...
im = ax_DTI.pcolor(log_conn, vmin=log_conn[log_conn > -np.inf].min().min(), vmax=log_conn.max().max(), cmap="YlGn")
...
# Fit
if has_macaque_data:
    ax_FIT.plot(macaque_logratio, macaque_sln, ".", markersize=4, color="silver", label="data")
...
ax_DIR.axis("square")
ax_DIR.invert_yaxis()
cbar = fig.colorbar(im, ax=ax_DIR, shrink=0.7, pad=-0.243)
cbar.set_ticks([0, 1])
...
areas = [dk_full_to_short[x] for x in areas]
im = ax_MESO.pcolor(
    log_synapses, vmin=log_synapses[log_synapses > -np.inf].min().min(), vmax=log_synapses.max().max(), cmap="YlGn"
)
