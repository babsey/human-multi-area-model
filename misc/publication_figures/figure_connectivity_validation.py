...
# Community structure
synapses_area = synapses.groupby("area", axis="index").sum().groupby("area", axis="columns").sum()
...
directionality_inflated = pd.DataFrame(dtype=str, index=outdeg.index, columns=outdeg.columns)
for source, target in product(area_list, area_list):
    directionality_inflated.loc[(target, slice(None), slice(None)), (source, slice(None), slice(None))] = (
...
fig = plt.figure(constrained_layout=True, figsize=(5.63, 3.0))
...
ax_synapses.text(s="A", transform=ax_synapses.transAxes, x=-0.1, y=1.2, **label_prms)
...
ax_edr.text(s=r"$\lambda$" + f" = {edr_decay:.1f} mm", x=0.6, y=0.8, transform=ax_edr.transAxes)
...
ax_outdeg.text(s=r"$\mu_\mathrm{FF}$" + f" = {mean_outdeg_FF:.0f}", x=0.55, y=0.85, transform=ax_outdeg.transAxes)
ax_outdeg.text(s=r"$\mu_\mathrm{FB}$" + f" = {mean_outdeg_FB:.0f}", x=0.55, y=0.75, transform=ax_outdeg.transAxes)
...
ax_outdeg.text(s="C", transform=ax_outdeg.transAxes, x=-0.1, y=1.25, **label_prms)

# Targts
x_max = max(targets_FF.max(), targets_FB.max(), targets_lat.max())
bins = 2 * np.arange(x_max // 2 + 2)
ax_targets.hist(
    [targets_FF, targets_lat, targets_FB],
    bins=bins,
    log=True,
    density=True,
...
ax_targets.text(s=r"$\mu_\mathrm{FF}$" + f" = {mean_targets_FF:.2f}", x=0.5, y=0.9, transform=ax_targets.transAxes)
ax_targets.text(s=r"$\mu_\mathrm{LAT}$" + f" = {mean_targets_lat:.2f}", x=0.5, y=0.8, transform=ax_targets.transAxes)
ax_targets.text(s=r"$\mu_\mathrm{FB}$" + f" = {mean_targets_FB:.2f}", x=0.5, y=0.7, transform=ax_targets.transAxes)
...
ax_targets.text(s="D", transform=ax_targets.transAxes, x=-0.1, y=1.25, **label_prms)
