...
from .compute_scaling_experiments_helpers import (
    LVr,
    calculateFuncionalConnectivityCorrelations,
    cvIsi,
    get_cc_scalingEtoE,
    get_neuron,
    json_dump,
...
initial_transient = 2500.0
exp_st_duration = 2000.0
tmax = 12500.0
...
for type_, hash_ in state.items():
    ...
    for j in range(first_snippet, number_of_simulated_2s_snippets):
        ...
        for st in spks:
            st = st[
                (st > initial_transient + i * exp_st_duration) & (st < initial_transient + (i + 1) * exp_st_duration)
            ]
            sim_rates[type_].append(st.size / t_max)
            if st.size >= LvR_min_spikes:
                isi = np.diff(st)
                if isi.min() > 0:
                    sim_lvr[type_].append(lvr_from_isi(isi, tau_r=2.0))
            if st.size >= CV_min_spikes:
                isi = np.diff(st)
                if isi.min() > 0:
...
for d in path:
    ...
    if os.path.isdir(p_net):
        ...
        cv, _ = cvIsi(spks, CV_min_spikes=CV_min_spikes)

        lv, _ = LVr(spks, 2, LvR_min_spikes=LvR_min_spikes)
...
for use_corrcoeff in [True, False]:
    ...
    for d in path:
        ...
        if os.path.isdir(p_net):
            ...
            scale.append(chi)
            exp__sim_syn__corr_lh, exp__sim_syn__corr_rh, exp_fc_mean_lh, exp_fc_array_rh = (
                calculateFuncionalConnectivityCorrelations(
                    p_ana,
                    tmin=initial_transient,
                    tmax=tmax,
                    use_corrcoeff=use_corrcoeff,
