...
replace_this = ["PERIRHINAL", "8L", "8B", "CORE", "ENTORHINAL", "INSULA", "TEMPORAL_POLE", "V3A"]
...
density_ratio = pd.DataFrame(
    data=dens_target / dens_source, index=density_data["area"], columns=density_data["area"]
).unstack()
...
# Merge data & export
merged_data = (
    fln_data.merge(sln_data, how="left", on=["SOURCE", "TARGET"])
    .merge(density_ratio, how="left", on=["SOURCE", "TARGET"])
    .dropna()
)
